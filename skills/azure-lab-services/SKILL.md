...
|----------|-------|-------------|
| Troubleshooting | L33-L40 | Diagnosing and fixing Azure Lab Services issues like VM connectivity/access failures and lab creation errors, including reset options and common connection problem resolutions. |
| Best Practices | L41-L48 | Guidance on building and configuring lab template images (Windows/Linux), enabling nested virtualization, and tuning remote desktop performance for Azure Lab Services. |
...
| Architecture & Design Patterns | L58-L63 | Azure Lab Services core architecture concepts, components, and supported networking/topology options for designing and selecting lab plan network setups. |
| Limits & Quotas | L64-L72 | VM capacity/core quotas, how to measure and request more cores, set user access and lab usage hours, and understand VM inactivity and auto-deletion policies. |
| Security | L73-L87 | RBAC roles, lab owner/creator assignment, Azure Policy controls, VM size restrictions, firewall and public IP configuration, and password reset options for Azure Lab Services. |
| Configuration | L88-L111 | Configuring Azure Lab Services labs and lab plans: regions, images, networking, VM schedules/shutdown, GPU/nested virtualization, policies, and automation via ARM/Bicep/PowerShell/Python. |
| Integrations & Coding Patterns | L112-L123 | Integrating Lab Services with Canvas and Teams, connecting labs to VNets, and automating lab creation/management via Bicep, PowerShell, Python SDK, and X2Go Linux access. |
| Deployment | L124-L133 | Creating and migrating labs and lab plans, including importing Windows/Linux images from VMs or physical labs, and setting up/publishing Azure Lab Services environments. |
