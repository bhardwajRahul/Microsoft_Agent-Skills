...
|----------|-------|-------------|
| Configuration | L27-L31 | Configuring Jenkins with Azure plugins to integrate builds and deployments with Azure services (VMs, App Service, storage, credentials, and service principals). |
| Integrations & Coding Patterns | L32-L36 | Configuring Jenkins to store build artifacts in Azure Blob Storage, including setup, authentication, and using Blob containers as an external artifact repository. |
| Deployment | L37-L49 | Jenkins deployment guides for Azure: setting up Jenkins on VMs/containers and building CI/CD pipelines to deploy apps to AKS, App Service, Functions, Spring Apps, Service Fabric, and Linux VMs. |
