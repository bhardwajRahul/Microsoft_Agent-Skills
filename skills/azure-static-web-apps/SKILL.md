...
| Decision Making | L36-L44 | Guidance on choosing hosting plans, Functions integration model, Next.js deployment mode, and Front Door CDN options for Azure Static Web Apps. |
| Limits & Quotas | L45-L50 | Runtime and language support matrix plus app size, bandwidth, API, build, and environment quotas and limits for Azure Static Web Apps. |
| Security | L51-L64 | Authentication, authorization, roles, secrets, private access, and secure connections for Static Web Apps, including Entra ID, custom providers, Key Vault, and user info access |
| Configuration | L65-L88 | Configuring domains, routing, builds, and local dev for Static Web Apps, plus CLI usage, backend app settings, monitoring (App Insights), and Nuxt/front-end framework build settings |
| Integrations & Coding Patterns | L89-L100 | Patterns for wiring Static Web Apps to backends and data: API Management, App Service, Container Apps, and connecting to Cosmos DB, SQL, MySQL, PostgreSQL (incl. Mongoose). |
| Deployment | L101-L114 | Deploying Static Web Apps via GitHub/GitLab/Bitbucket/CLI/ARM/Bicep, plus configuring preview environments, PR reviews, and traffic splitting for staged releases |
