...
category_descriptions:
  ...
  security: 'Securing Azure Key Vault and Managed HSM: auth, RBAC vs access policies,
    firewalls, Private Link, soft-delete, backups, and security best practices for
    keys, secrets, and certificates.'
  configuration: 'Configuring Key Vault and Managed HSM: monitoring, alerts, logging,
    policies, key types/rotation, secure key release, replication, and special secret
    formats (e.g., multiline).'
  ...
  limits-quotas: 'Key Vault/Managed HSM limits: throttling, quotas, size constraints,
    logging latency, soft-delete behavior, and network/IP firewall configuration.'
  ...
  best-practices: Guidance on BYOK/HSM key import, key/secret security best practices,
    disaster recovery for Managed HSM, and automating single/dual-credential secret
    rotation in Key Vault.
...
skill_description: Expert knowledge for Azure Key Vault development including troubleshooting,
  ...
  & coding patterns, and deployment. Use when using Key Vault/Managed HSM APIs, RBAC
  vs access policies, Private Link, key rotation, or IaC deployment, and other Azure
...
use_when: Use when using Key Vault/Managed HSM APIs, RBAC vs access policies, Private
  Link, key rotation, or IaC deployment, and other Azure Key Vault related development
  tasks.
...
| deployment | 1 | 0.6% |
| integrations | 24 | 14.8% |
| limits-quotas | 7 | 4.3% |
| security | 22 | 13.6% |
| troubleshooting | 5 | 3.1% |
| *(Unclassified)* | 73 | 45.1% |
...
| [Managed HSM role management](https://learn.microsoft.com/en-us/azure/key-vault/managed-hsm/role-management) | security | 0.80 | Role management for Managed HSM data plane will list specific built-in role names, scopes, and CLI commands for assignments, which are product-specific security configuration details. |
...
| [Backup and restore and selective key restore](https://learn.microsoft.com/en-us/azure/key-vault/managed-hsm/backup-restore) | security | 0.70 | Describes backup/restore operations including specific data plane permission (Microsoft.KeyVault/managedHsm/backup/start/action) and likely other operation names and requirements; these are product-specific security and access configuration details. |
...
| [Enable Multi-Region Replication on Managed HSM](https://learn.microsoft.com/en-us/azure/key-vault/managed-hsm/multi-region-replication) | configuration | 0.70 | Describes product-specific behavior and constraints for enabling multi-region replication (primary vs extended region, only one additional region, both regions active, replication behavior). These are concrete service behaviors/configuration characteristics that go beyond generic concepts, but it doesn’t appear to focus on numeric limits or decision matrices. |
...
| [Disaster recovery](https://learn.microsoft.com/en-us/azure/key-vault/managed-hsm/disaster-recovery-guide) | best-practices | 0.65 | Provides a step-by-step disaster recovery procedure for Managed HSM, mapping failure scenarios to specific recovery steps and configuration changes; these are product-specific operational best practices. |
...
| [Back up and restore objects](https://learn.microsoft.com/en-us/azure/key-vault/general/backup) | 0.45 | Backup article is likely a procedural how-to using portal/CLI/PowerShell; summary doesn’t indicate detailed parameter tables, limits, or error mappings beyond generic backup steps. |
...
| [Manage with the Azure CLI](https://learn.microsoft.com/en-us/azure/key-vault/managed-hsm/key-management) | 0.35 | Key management article for Managed HSM appears to be procedural/overview; summary does not indicate detailed configuration tables, limits, or error/role specifics. |
